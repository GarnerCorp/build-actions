...
SEGMENT = re.compile(r"[^A-Za-z0-9]+")
...
def repository_names(head):
    listing = subprocess.run(["git", "ls-tree", "-r", "--name-only", head],
                             capture_output=True, text=True, check=True).stdout
    return {segment.lower() for name in listing.split("\n")
            for segment in SEGMENT.split(name) if segment}
...
def plan(work_dir):
    ...
    Path(f"{work_dir}/names.txt").write_text(
        "\n".join(sorted(repository_names(head))), encoding="utf-8")
...
def grounded(findings, sent, names):
    kept, seen = [], set()
...
def review(work_dir):
    ...
    names = set(Path(f"{work_dir}/names.txt").read_text(encoding="utf-8").split())
    kept = grounded(findings, sent_lines(parts), names)
